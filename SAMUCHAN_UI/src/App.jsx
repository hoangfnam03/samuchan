...
function ShopPage({ shopName = DEFAULT_SHOP_NAME, skuMaster, orders, exchangeRate, formatVnd, sales, onSalesChange, skuLinks, salesLoading, salesError }) {
  ...
  const [expandedProductSku, setExpandedProductSku] = useState(null)
  ...
  const getTopBuyers = (skuId) => {
    const buyers = new Map()
    filteredSales.forEach((sale) => {
      if (String(sale.sku) !== String(skuId)) return
      const displayName = String(sale.buyerName || '').trim() || 'Chưa rõ người mua'
      const key = displayName.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
      const buyer = buyers.get(key) || { name: displayName, quantity: 0 }
      buyer.quantity += getSaleDetails(sale).quantity
      buyers.set(key, buyer)
    })
    return [...buyers.values()].sort((a, b) => b.quantity - a.quantity || a.name.localeCompare(b.name)).slice(0, 5)
  }
  ...
  return <main className="dashboard">
    ...
    <section className="orders-card shop-product-chart"><div className="shop-chart-heading"><div><p className="eyebrow">SẢN PHẨM BÁN CHẠY</p><h2>Xếp hạng theo số lượng bán</h2></div><small>Biểu đồ thay đổi theo bộ lọc ngày phía trên. Lợi nhuận đã tính giá vốn và phí vận chuyển của từng đơn.</small></div>{productRows.length ? <div className="shop-product-rows">{productRows.map((row) => { const isExpanded = expandedProductSku === row.sku; const topBuyers = isExpanded ? getTopBuyers(row.sku) : []; return <div className={`shop-product-row ${isExpanded ? 'is-expanded' : ''}`} key={row.sku} role="button" tabIndex="0" aria-expanded={isExpanded} onClick={() => setExpandedProductSku(isExpanded ? null : row.sku)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setExpandedProductSku(isExpanded ? null : row.sku) } }}><div className="shop-product-label"><strong>{row.sku}</strong><small>{row.name}</small></div><div className="shop-product-quantity"><div className="shop-product-track"><span className="shop-product-bar" style={{ width: `${row.quantity / productChartMax * 100}%` }} /></div><b>{row.quantity} cái</b></div><div className="shop-product-metric shop-product-revenue"><small>Doanh thu</small><strong>{formatVnd(row.revenue)}</strong></div><div className={`shop-product-metric shop-product-profit ${row.profit >= 0 ? 'is-positive' : 'is-negative'}`}><small>Lợi nhuận</small><strong className={row.profit >= 0 ? 'profit-positive' : 'profit-negative'}>{formatVnd(row.profit)}</strong></div>{isExpanded && <div className="shop-product-buyers" onClick={(event) => event.stopPropagation()}><div className="shop-product-buyers-heading"><strong>Top 5 người mua nhiều nhất</strong><small>{row.sku} · {row.quantity} cái đã bán</small></div>{topBuyers.length ? <div className="shop-product-buyer-list">{topBuyers.map((buyer, index) => <div className="shop-product-buyer" key={`${row.sku}-${buyer.name}-${index}`}><span><b>{index + 1}</b>{buyer.name}</span><strong>{buyer.quantity} cái</strong></div>)}</div> : <small className="shop-product-buyers-empty">Chưa có thông tin người mua.</small>}</div>}</div> })}</div> : <div className="empty-state">Chưa có dữ liệu sản phẩm trong khoảng thời gian này.</div>}</section>
